...
//String [] solfege = {"Do", "Di", "Re", "Ri", "Mi", "Fa", "Fi", "Sol", "Si", "La", "Li", "Ti"};
    ...
    public void setup() {
        ...
        loadInfo();
        scalars = width / idealW;

        float idealH = height * aspectRatio;
        fft = new CustomFFT(this, bands);
    ...
    public boolean intersects() {
        ...
        //fill(255, 100);
        //ellipse(sin(start)*(2*idealW/3), cos(start)*2*(idealW/3),sin(end)*(2*idealW/3), cos(end)*2*(idealW/3))
        //The ball is close to the paddle? (the distance from the center of the paddle is less than the length of the paddle.)
        return dist(centerX, centerY, ballX, ballY) < dist(cos(start) * (idealW / 3) + idealW / 2, sin(start) * (idealW / 3) + idealH / 2 + idealH / 10, cos(end) * ((idealW / 3)) + idealW / 2, sin(end) * (idealW / 3) + idealH / 2 + idealH / 10);
    }
    ...
    public void drawBall() {
        ...
        fill(255, 100);
        //stroke(255,255,0);
        //line(cos(start)*(idealW/3)+idealW/2, sin(start)*(idealW/3)+idealH/2+idealH/10,cos(end)*((idealW/3))+idealW/2, sin(end)*(idealW/3)+idealH/2+idealH/10);
        if (dist(idealW / 2, idealH / 2 + idealH / 10, ballX + ballSpeedX, ballY + ballSpeedY) > ((idealW - idealW / 3)) / 2 - idealW / 50 && intersects()) {
            int randomSolfege = 0;
            tail = 0;
    ...
    public void drawBackground() {
        ...
        for (int i = 0; i < solfege.length; i++) {
            ...
            text(solfege[i], cos((((TWO_PI) / solfege.length) * i) + HALF_PI + PI) * rad / 2 * 1.02f + idealW / 2 - idealW / 35, sin(((TWO_PI) / solfege.length) * i + HALF_PI + PI) * rad / 2 * 1.01f + idealH / 2 + idealH / 9);
        }
        noFill();
        ...
        public void colorBars() {
            ...
            for (int i = 0; i < spectrum.length; i++) {
                ...
                if (i > 27) {
                    ...
                    line(i * scale, idealH, i * scale, idealH - spectrum[i] * idealH * amplify);
                }
                colorMode(RGB, 255);
            }
        }
